...
public class foodSelect extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Toast.makeText(getApplicationContext(), "아침 식사 정보", Toast.LENGTH_SHORT).show();

        intent=getIntent();
        arrayFoodList=(ArrayList<food>)intent.getSerializableExtra("Foodname");
        Serializable s=intent.getSerializableExtra("Foodname");

        toal_kcal=(TextView)findViewById(R.id.tv_total_kcal);
        ...
        //리사이클러뷰 초기화
        mRecyclerView = findViewById(R.id.recyclerView2);// 리사이클러뷰 레이아웃 연결
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        switch (item.getItemId()) {
            ...
            case R.id.toolbar_next_button:  // 오른쪽 상단 버튼 눌렀을 때
                ...
                firebaseFoodInput post = new firebaseFoodInput(email_id,"아침",format_time,totalCalcium,totalCarbon,totalFat,totalIron,totalFiber,totalSugar,totalVitaminC,totalVitaminB1,totalVitaminB2,totalKcal,totalNa,totalProtein );
                proValues = post.toMap();

                childUpdates.put("/MorningFood/" + email_id, proValues);
